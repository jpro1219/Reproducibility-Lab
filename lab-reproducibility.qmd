...
precision_logit <- TP/(TP + FP) # Apply formula for precision
accuracy_logit <- (TP + TN) / sum(logit_conf_30[,3])
...
precision_logit <- TP/(TP + FP) # Apply formula for precision 
accuracy_logit <- (TP + TN) / sum(logit_conf_40[,3])
...
model_ratings <- data.frame(models, accuracy, precision)
...
ggarrange(Accuracy_plot, nrow = 1, ncol = 2)
ggsave("Figures/Figure3_Scoring.png", width = 20, height = 10)

scoring <- data.frame(Model=c("Accuracy", "Precision"), Bayes = c(accuracy_bayes, F2_bayes), Logit=c(accuracy_logit, f2_logit), LDA = c(accuracy_lda, F2_lda), RF = c(accuracy_rf, F2_rf))
